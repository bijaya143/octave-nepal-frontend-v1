...
const related: Course[] = [
  ...
  },
  {
    ...
    price: 109,
    discount: 20,
    thumbnail: "https://images.unsplash.com/photo-1527980965255-d3b416303d12?auto=format&fit=crop&w=1200&q=80",
  },
];
  ...
  return (
    <main>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-6 md:gap-8 items-start">
        {/* Content */}
        <section>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-balance mb-2" style={{ fontFamily: "var(--font-heading-sans)" }}>{course.title}</h1>
          <div className="flex items-center gap-3 mb-6">
            <span className="flex items-center gap-2 text-sm text-[color:var(--color-neutral-700)]">
              {course.instructorAvatar && (
                <Image src={course.instructorAvatar} alt={`${course.instructor} avatar`} width={20} height={20} className="h-5 w-5 rounded-full object-cover" />
              )}
              By {course.instructor}
            </span>
            <Rating value={course.rating} count={course.ratingCount} />
            <span className="text-sm text-[color:var(--color-neutral-600)]">• {course.duration}</span>
          </div>

          <Card className="overflow-hidden mb-6">
            <div className="relative aspect-[16/9] w-full">
              <Image src={course.thumbnail || "/images/thumb-1.svg"} alt="Course thumbnail" fill sizes="(min-width: 1024px) 66vw, 100vw" className="object-cover" />
            </div>
          </Card>

          <section className="space-y-4">
            <h2 className="text-lg font-semibold" style={{ fontFamily: "var(--font-heading-sans)" }}>Description</h2>
            <p className="text-sm text-[color:var(--color-neutral-700)]">
              Build real-world React 19 apps using modern patterns, server actions, and performance best practices.
              Learn by doing with hands-on projects.
            </p>
          </section>

          <section className="space-y-3 mt-8">
            <h2 className="text-lg font-semibold" style={{ fontFamily: "var(--font-heading-sans)" }}>Syllabus</h2>
          ...
          <section className="mt-8">
            <h2 className="text-lg font-semibold mb-3" style={{ fontFamily: "var(--font-heading-sans)" }}>Reviews</h2>
            <Card>
              <CardContent className="py-5">
                <div className="space-y-4">
                  {reviews.map((r) => (
                    <div key={r.name} className="border-b last:border-0 border-[color:var(--color-neutral-200)] pb-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <Image src={r.avatar} alt={`${r.name} avatar`} width={32} height={32} className="h-8 w-8 rounded-full object-cover" />
                          <span className="text-sm font-medium">{r.name}</span>
                        </div>
                        <Rating value={r.rating} />
                      </div>
                      <p className="text-sm text-[color:var(--color-neutral-700)] mt-1">{r.text}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          <Card>
            <CardContent className="py-5">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-2xl text-[color:var(--color-primary-700)] font-bold">Rs 84</span>
                <span className="text-sm line-through text-[color:var(--color-neutral-500)]">Rs 129</span>
              </div>
              <Link href={`/checkout?courseId=${id}`} className="block">
                <Button className="w-full">Enroll now</Button>
              </Link>
              <p className="text-xs text-[color:var(--color-neutral-600)] mt-3">30-day money-back guarantee</p>
            </CardContent>
          </Card>

          <div className="mt-8">
            <h3 className="text-base font-semibold mb-3" style={{ fontFamily: "var(--font-heading-sans)" }}>Related courses</h3>
