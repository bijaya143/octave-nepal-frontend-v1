...
export default function BlogPostViewModal({ post, onClose }: Props) {
  return (
    <Modal
      open={!!post}
      onClose={onClose}
      title={post ? "Blog Post Details" : undefined}
    >
      {post && (
        <div className="space-y-4 text-sm max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="inline-flex items-center gap-1 rounded-md border border-[color:var(--color-neutral-200)] bg-white px-2 py-0.5 font-mono text-[11px] text-[color:var(--color-neutral-700)]">
                /{post.slug}
              </div>
            </div>
            <Image
              ...
              width={120}
              height={80}
              className="rounded-lg ring-1 ring-[color:var(--color-neutral-200)] shadow-sm"
            />
          </div>
            ...
            <div className="rounded-lg border border-[color:var(--color-neutral-200)] bg-white p-3">
              ...
              <div className="flex flex-wrap gap-1">
                {post.tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-[color:var(--color-neutral-100)] text-[color:var(--color-neutral-700)] text-xs"
                  >
                    <Tag size={10} />
                    {tag}
                  </span>
