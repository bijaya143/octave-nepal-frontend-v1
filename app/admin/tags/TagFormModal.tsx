...
export default function TagFormModal({
	...
	const [values, setValues] = React.useState<TagFormValues>({ ...DEFAULTS, ...initialValues });
	...
	function handleChange<K extends keyof TagFormValues>(key: K, value: TagFormValues[K]) {
	...
	return (
		<Modal open={open} onClose={onClose} title={title ?? (mode === "edit" ? "Edit Tag" : "Create Tag")}>
			<form onSubmit={handleFormSubmit} noValidate className="space-y-5 text-sm">
				...
				{activeTab === "details" && (
					<div className="space-y-3">
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
							<Input label="Name" value={values.name} onChange={handleNameChange} placeholder="Tag name" required />
							<div>
								<Input label="Slug" value={values.slug} onChange={(e) => handleChange("slug", e.target.value)} placeholder="tag-slug" required />
						...
						<div className="space-y-2">
							<Textarea label="Description" value={values.description || ""} onChange={(e) => handleChange("description", e.target.value)} placeholder="Short description" rows={3} />
						</div>
					</div>
				)}
				...
				{activeTab === "media" && (
					<div className="space-y-3">
						<div className="rounded-lg border border-dashed border-[color:var(--color-neutral-300)] bg-[color:var(--color-neutral-50)] p-3">
							<label htmlFor="tag-image-input" className="block text-xs text-[color:var(--color-neutral-600)] mb-2">Tag Image</label>
							<div className="flex items-center gap-3">
								...
								<div className="flex-1">
									<p className="text-xs text-[color:var(--color-neutral-600)]">Upload a square image for this tag.</p>
									<div className="mt-2 flex items-center gap-2">
										<input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" id="tag-image-input" />
				...
				{activeTab === "visibility" && (
					<div className="space-y-3">
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
							<Select label="Status" hint="Controls whether this tag is visible to users." value={values.isPublished ? "Published" : "Unpublished"} onChange={(e) => handleChange("isPublished", e.target.value === "Published")}>
