      ...
      <section className="relative py-12 sm:py-14 md:py-16">
        ...
        <Container>
          <div className="relative">
            {/* Header */}
            <div className="mb-3"><Badge variant="outline">Meet the team</Badge></div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight" style={{ fontFamily: "var(--font-heading-sans)" }}>The people behind {SITE_NAME}</h1>
            <p className="mt-2 text-[color:var(--color-neutral-600)] text-sm md:text-base max-w-3xl">A focused leadership team shaping vision, craft, and community.</p>
                  ...
                  <div className="relative rounded-2xl p-[1px] bg-gradient-to-br from-[rgba(59,130,246,0.35)] via-[rgba(147,51,234,0.25)] to-[rgba(16,185,129,0.3)]">
                    <Card className="p-0 overflow-hidden rounded-2xl border-transparent bg-white/90 backdrop-blur-sm shadow-sm">
                      <div className="relative h-[16rem] sm:h-[18rem] md:h-[21rem] w-full">
                        <Image src={m.image} alt={`${m.name} portrait`} fill sizes="(max-width: 1024px) 100vw, 25vw" className="object-cover" />
                      ...
                      <CardContent className="py-4 sm:py-5">
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          <div className="text-sm sm:text-base md:text-lg font-semibold break-words" style={{ fontFamily: "var(--font-heading-sans)" }}>{m.name}</div>
                        </div>
                        <div className="text-[11px] sm:text-sm text-[color:var(--color-primary-700)] mt-0.5 sm:mt-1">{m.role}</div>
                      </CardContent>
                    </Card>
                  </div>
                </motion.div>
            ...
            <div className="mt-12 md:mt-14">
              <div className="mb-3"><Badge variant="outline">Instructors</Badge></div>
              <h2 className="text-xl md:text-2xl font-semibold" style={{ fontFamily: "var(--font-heading-sans)" }}>Learn from practitioners</h2>
              <p className="mt-1 text-sm text-[color:var(--color-neutral-600)]">Expert mentors with real-world experience and clear teaching styles.</p>
                    ...
                    <div className="relative rounded-2xl p-[1px] bg-gradient-to-br from-[rgba(99,102,241,0.25)] via-[rgba(59,130,246,0.22)] to-[rgba(16,185,129,0.22)]">
                      <Card className="p-0 overflow-hidden rounded-2xl border-transparent bg-white/95 backdrop-blur-sm shadow-sm">
                        <div className="relative h-[13.5rem] sm:h-[15.5rem] md:h-[16rem] w-full">
                          <Image src={m.image} alt={`${m.name} portrait`} fill sizes="(max-width: 1280px) 100vw, 25vw" className="object-cover" />
                        ...
                        <CardContent className="py-3.5 sm:py-5">
                          <div className="text-sm sm:text-base md:text-lg font-semibold" style={{ fontFamily: "var(--font-heading-sans)" }}>{m.name}</div>
                          <div className="text-[11px] sm:text-xs text-[color:var(--color-primary-700)] mt-0.5 sm:mt-1">{m.role}</div>
                          ...
                          <div className="mt-2.5 rounded-lg border border-[color:var(--color-neutral-200)]/70 bg-[color:var(--color-neutral-50)]/60 px-2.5 sm:px-3 py-1.5 sm:py-2 flex flex-wrap items-center gap-1.5 sm:gap-2">
                            {m.socials?.linkedin && (
                              <Link href={m.socials.linkedin} aria-label={`${m.name} on LinkedIn`} className="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-md border border-[color:var(--color-neutral-200)] hover:bg-[color:var(--color-neutral-100)]"><Linkedin className="h-3.5 w-3.5 sm:h-4 sm:w-4" /></Link>
                            )}
                            {m.socials?.twitter && (
                              <Link href={m.socials.twitter} aria-label={`${m.name} on X/Twitter`} className="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-md border border-[color:var(--color-neutral-200)] hover:bg-[color:var(--color-neutral-100)]"><Twitter className="h-3.5 w-3.5 sm:h-4 sm:w-4" /></Link>
                            )}
                            {m.socials?.github && (
                              <Link href={m.socials.github} aria-label={`${m.name} on GitHub`} className="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-md border border-[color:var(--color-neutral-200)] hover:bg-[color:var(--color-neutral-100)]"><Github className="h-3.5 w-3.5 sm:h-4 sm:w-4" /></Link>
                            )}
                            {m.socials?.email && (
                              <Link href={m.socials.email} aria-label={`Email ${m.name}`} className="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-md border border-[color:var(--color-neutral-200)] hover:bg-[color:var(--color-neutral-100)]"><Mail className="h-3.5 w-3.5 sm:h-4 sm:w-4" /></Link>
                            )}
                          </div>
                        </CardContent>
                      </Card>
              ...
              <Card className="relative overflow-hidden">
                ...
                <CardContent className="relative py-7 md:py-8">
                  <div className="grid md:grid-cols-2 items-center gap-6 md:gap-10">
                    <div>
                      <h2 className="text-lg md:text-xl font-semibold" style={{ fontFamily: "var(--font-heading-sans)" }}>We’re hiring</h2>
                      <p className="mt-2 text-sm text-[color:var(--color-neutral-600)]">Join us in building modern, outcome-focused courses for learners across Nepal.</p>
                    </div>
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:justify-end">
                      <Link href="/careers" className="block w-full sm:w-auto">
                        <Button size="lg" className="w-full sm:w-auto">See open roles</Button>
                      </Link>
                      <Link href="/contact" className="block w-full sm:w-auto">
                        <Button size="lg" variant="secondary" className="w-full sm:w-auto">Partner with us</Button>
                      </Link>
                    </div>
                  </div>
                </CardContent>
