...
function ResetPasswordFormComponent() {
  ...
  useEffect(() => {
    const savedTimer = localStorage.getItem(
      "instructorResetPasswordCountdownEnd"
    );
    if (savedTimer) {
      ...
      if (endTime > now) {
        ...
        localStorage.removeItem("instructorResetPasswordCountdownEnd");
      }
    } else {
      setResendTimer(0);
    }
  ...
  const handleResetPassword = async (
    ...
    try {
      // Call the instructor reset password API
      const response = await instructorAuthService.resetPassword({
        email,
        token: otp,
        password,
      });
  ...
  const handleResendOTP = async () => {
    ...
    setIsResending(true);
    try {
      const response = await instructorAuthService.forgotPassword({ email });
